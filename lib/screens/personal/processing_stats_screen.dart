...
/// Universal Processing Statistics Screen
///
/// Shows detailed statistics about transaction processing for both SMS and Email:
...
class ProcessingStatsScreen extends StatefulWidget {
  final TransactionSource? initialSource; // null = all sources

  const ProcessingStatsScreen({
    super.key,
    this.initialSource,
  });

  @override
  State<ProcessingStatsScreen> createState() => _ProcessingStatsScreenState();
}

class _ProcessingStatsScreenState extends State<ProcessingStatsScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController = TabController(
      length: 3,
      vsync: this,
      initialIndex: _selectedSource == TransactionSource.sms ? 0 : (_selectedSource == TransactionSource.email ? 1 : 2),
    );
  ...
  void _onTabChanged() {
    if (!_tabController.indexIsChanging) {
      setState(() {
        _selectedSource = _tabController.index == 0
            ? TransactionSource.sms
            : (_tabController.index == 1 ? TransactionSource.email : null);
      });
      _loadStats();
    }
  }
  ...
  Future<void> _loadStats() async {
    ...
    try {
      ...
      // Load transactions for selected source
      final transactions = await LocalDBService.instance.getTransactions(
        userId: currentUser.uid,
        source: _selectedSource,
      );

      // Load all patterns (patterns work across sources based on sender)
      ...
      final patternMatched = transactions.where((t) => t.parsedBy == ParseMethod.regex).length;
      final aiProcessed = transactions.where((t) => t.parsedBy == ParseMethod.ai).length;
      final totalProcessed = transactions.length;

      // Source-specific breakdown
      final smsTxns = transactions.where((t) => t.source == TransactionSource.sms).length;
      final emailTxns = transactions.where((t) => t.source == TransactionSource.email).length;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: const [
            Tab(text: 'SMS', icon: Icon(Icons.sms, size: 20)),
            Tab(text: 'Email', icon: Icon(Icons.email, size: 20)),
            Tab(text: 'All', icon: Icon(Icons.analytics, size: 20)),
          ],
        ),
      ),
      body: _isLoading
  ...
  Widget _buildSourceIndicator(ThemeData theme) {
    final sourceColor = _selectedSource == TransactionSource.sms ? Colors.green : Colors.blue;
    final sourceIcon = _selectedSource == TransactionSource.sms ? Icons.sms : Icons.email;
    final sourceName = _selectedSource == TransactionSource.sms ? 'SMS' : 'Email';
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Text(
            'Showing $sourceName transactions only',
            style: TextStyle(
  ...
  Widget _buildOverviewCard(ThemeData theme) {
    ...
    final smsTxns = _stats['smsTxns'] ?? 0;
    final emailTxns = _stats['emailTxns'] ?? 0;
        ...
        child: Column(
          ...
          children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        _selectedSource == null ? 'All sources' : (_selectedSource == TransactionSource.sms ? 'SMS only' : 'Email only'),
                        style: TextStyle(
                          fontSize: 13,
            ...
            if (_selectedSource == null) ...[
              ...
              Row(
                children: [
                  const SizedBox(width: 44), // Indent to align with stat values
                  Expanded(
                    child: Row(
                      children: [
                        ...
                        Text(
                          'SMS: ${NumberFormat('#,##0').format(smsTxns)}',
                          style: TextStyle(
                            fontSize: 13,
                        ...
                        const SizedBox(width: 8),
                        Text(
                          'Email: ${NumberFormat('#,##0').format(emailTxns)}',
                          style: TextStyle(
                            fontSize: 13,
                            color: theme.textTheme.bodyMedium?.color,
                          ),
  ...
  Widget _buildPatternItem(
    ...
    final sourceIcon = source == 'sms' ? Icons.sms : Icons.email;
    final sourceColor = source == 'sms' ? Colors.green : Colors.blue;
